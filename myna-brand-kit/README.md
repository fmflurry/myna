...
The `myna` wordmark is lowercase Poppins Semibold with tighter tracking
...
This pairing keeps the brand friendly and rounded while preserving excellent
readability in dense meeting/transcription interfaces.
...
Yellow is an accent, not a large background color. Prefer Ink + Off White as
the UI foundation.
...
Myna self-hosts WOFF2 (`ui/src/styles.scss`); never `@import` from a CDN in
the app. The CSP is `default-src 'self'` and the product promise is fully
local.

## Logo usage

- Keep clear space around the logo roughly equal to the height of the
  lowercase `m` stem.
